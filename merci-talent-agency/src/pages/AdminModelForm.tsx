...
export default function AdminModelForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto-add MO tag if category is models
      let finalTags = [...(restData.tags || [])];
      if (restData.category === 'models' && !finalTags.includes('MO（モデル）')) {
        finalTags.push('MO（モデル）');
      }
      ...
      if (id) {
        const { error } = await supabase
          ...
          .eq('id', id);
        if (error) {
          console.error('Supabase error updating model:', error);
          throw error;
        }
      } else {
        const { error } = await supabase
        ...
        if (error) {
          console.error('Supabase error inserting model:', error);
          throw error;
        }
      }
      ...
      alert(`Error saving model: ${errorMessage}\n\n"column does not exist" というエラーが出る場合は、データベースのスキーマ（x_url列など）が最新か確認してください。`);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleModelMainImageUpload = (url: string) => {
    setFormData({ ...formData, model_main_image: url });
  };
  ...
  const handleModelPortfolioImageUpload = (url: string) => {
    setFormData({ ...formData, model_images: [...(formData.model_images || []), url] });
  };
  ...
  const removeModelImage = (index: number) => {
    const newImages = [...(formData.model_images || [])];
    newImages.splice(index, 1);
    setFormData({ ...formData, model_images: newImages });
  };
  ...
  const moveModelImage = (index: number, direction: 'up' | 'down') => {
    const newImages = [...(formData.model_images || [])];
    const targetIndex = direction === 'up' ? index - 1 : index + 1;
    if (targetIndex < 0 || targetIndex >= newImages.length) return;
  ...
  return (
    <div className="pt-32 pb-32 container-custom max-w-4xl">
      <button 
      ...
      <form onSubmit={handleSubmit} className="space-y-12">
                ...
                <div className="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  {i > 0 && (
                    <button 
                      ...
                      className="p-1 bg-brand-white/80 rounded-full hover:bg-brand-white transition-colors"
                      title="Move Left"
                    >
                      <ArrowLeft size={12} />
                    </button>
                  )}
                  {i < (formData.images?.length || 0) - 1 && (
                    <button 
                      type="button"
                      onClick={() => moveImage(i, 'down')}
                      className="p-1 bg-brand-white/80 rounded-full hover:bg-brand-white transition-colors rotate-180"
                      title="Move Right"
                    >
                      <ArrowLeft size={12} />
                    </button>
        ...
        {formData.tags?.includes('MO（モデル）') && (
          <div className="border-t-2 border-brand-black/20 pt-12 bg-brand-black/5 p-8 -mx-8">
            <div className="mb-8">
              <h3 className="text-sm tracking-[0.3em] font-bold uppercase">Model-Specific Images (MO用写真)</h3>
              <p className="text-[10px] text-brand-gray tracking-widest uppercase mt-2">
                These images will be shown when viewing the "MODELS" category page.
              </p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 mb-12">
              <div className="space-y-4">
                <label className="text-[10px] tracking-widest text-brand-gray uppercase block">MODEL MAIN IMAGE</label>
                {formData.model_main_image ? (
                  <div className="relative aspect-[3/4] bg-brand-black/5 group">
                    <img src={formData.model_main_image} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    <button 
                      type="button"
                      onClick={() => setFormData({ ...formData, model_main_image: '' })}
                      className="absolute top-2 right-2 p-2 bg-brand-white/80 rounded-full hover:bg-brand-white transition-colors opacity-0 group-hover:opacity-100"
                    >
                      <X size={14} />
                    </button>
                  ...
                  <ImageUpload 
                    onUpload={handleModelMainImageUpload} 
                    folder="models/main_model" 
                    customName={formData.name ? `${formData.name}_model_main` : undefined}
                  />
                )}
              </div>
            </div>
            ...
            <div className="space-y-8">
              <h3 className="text-xs tracking-widest text-brand-gray uppercase">MODEL PORTFOLIO IMAGES</h3>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {formData.model_images?.map((img, i) => (
                  <div key={i} className="relative aspect-[3/4] bg-brand-black/5 group">
                    <img src={img} className="w-full h-full object-cover" referrerPolicy="no-referrer" />
                    <div className="absolute top-2 right-2 flex space-x-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      {i > 0 && (
                      ...
                      {i < (formData.model_images?.length || 0) - 1 && (
                        <button 
                          type="button"
                          onClick={() => moveModelImage(i, 'down')}
                          className="p-1 bg-brand-white/80 rounded-full hover:bg-brand-white transition-colors rotate-180"
                          title="Move Right"
                        >
                          <ArrowLeft size={12} />
                ...
                <div className="aspect-[3/4]">
                  <ImageUpload 
                    onUpload={handleModelPortfolioImageUpload} 
                    folder="models/portfolio_model" 
                    customName={formData.name ? `${formData.name}_model_portfolio_${formData.model_images?.length || 0}` : undefined}
                  />
                </div>
              </div>
            </div>
