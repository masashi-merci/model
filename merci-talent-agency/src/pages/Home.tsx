...
export default function Home({ settings: initialSettings }: { settings: SiteSettings | null }) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          errorMessage = `Server error (${response.status}). The backend might not be running.`;
        }
  ...
  const categories = [
    ...
    { name: 'ANIMALS', slug: 'animals', image: settings?.animals_image || 'https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba?auto=format&fit=crop&q=80&w=1000' }
  ];
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-16 mb-32">
            ...
            >
              ...
              <p className="text-brand-gray text-sm leading-relaxed">
                {settings?.service_model_casting_text || 'ファッションショー、広告、カタログ、雑誌など、プロジェクトのコンセプトに最適なモデルをご提案します。国内外のネットワークを活かしたキャスティングが可能です。'}
              </p>
            ...
            >
              ...
              <p className="text-brand-gray text-sm leading-relaxed">
                {settings?.service_event_staffing_text || '展示会、プロモーションイベント、プライベートパーティーなど、華やかさとプロフェッショナリズムを兼ね備えたイベントスタッフ・コンパニオンを派遣します。'}
              </p>
            ...
            >
              ...
              <p className="text-brand-gray text-sm leading-relaxed">
                {settings?.service_production_text || 'スチール撮影から動画制作まで、クリエイティブディレクション、カメラマン、スタイリスト、ヘアメイクの手配を含めたトータルプロデュースを承ります。'}
              </p>
        ...
        <div className="container-custom">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-24 items-center">
            ...
            >
              ...
              <p className="text-brand-gray leading-relaxed mb-12 text-lg whitespace-pre-wrap">
                {settings?.about_text || `Merci Talent Agency. is more than just an agency; it's a creative hub where talent meets opportunity. 
          ...
          <div className="max-w-3xl mx-auto">
            ...
            <div className="space-y-12">
              <div className="space-y-6 text-center">
                <h3 className="text-3xl font-serif tracking-widest uppercase">Let's Connect</h3>
                <p className="text-brand-gray leading-relaxed max-w-2xl mx-auto">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="flex flex-col items-center text-center space-y-4">
                  <div className="p-4 bg-brand-black/5 rounded-full">
                    <MapPin size={24} className="text-brand-black" />
                  </div>
                  <div>
                    <h4 className="text-[10px] tracking-widest text-brand-gray uppercase mb-2">Address</h4>
                    <p className="text-brand-black font-medium text-sm">{settings?.address || "123 Fashion Ave, Shibuya, Tokyo, Japan"}</p>
                  </div>
                </div>

                <div className="flex flex-col items-center text-center space-y-4">
                  <div className="p-4 bg-brand-black/5 rounded-full">
                    <Phone size={24} className="text-brand-black" />
                  </div>
                  <div>
                    <h4 className="text-[10px] tracking-widest text-brand-gray uppercase mb-2">Phone</h4>
                    <p className="text-brand-black font-medium text-sm">{settings?.phone || "+81 03-6820-6049"}</p>
                  </div>
                </div>

                <div className="flex flex-col items-center text-center space-y-4">
                  <div className="p-4 bg-brand-black/5 rounded-full">
                    <Mail size={24} className="text-brand-black" />
                  </div>
                  <div>
                    <h4 className="text-[10px] tracking-widest text-brand-gray uppercase mb-2">Email</h4>
                    <p className="text-brand-black font-medium text-sm">{settings?.email || "event@thisismerci.com"}</p>
                  </div>
                </div>
              </div>

              {/* Contact Form */}
              <div className="bg-white p-8 md:p-12 rounded-2xl shadow-sm border border-brand-black/5">
                {submitted ? (
                  <motion.div 
                  ...
                  >
                    ...
                    <h3 className="text-2xl font-serif tracking-widest uppercase">Thank You</h3>
                    <p className="text-brand-gray text-sm tracking-widest uppercase">メッセージを承りました。内容を確認の上、担当者よりご連絡させていただきます。</p>
                    <button 
                      onClick={() => setSubmitted(false)}
                      className="text-[10px] tracking-[0.3em] border-b border-brand-black pb-1 uppercase hover:text-brand-gray hover:border-brand-gray transition-all"
                    >
                  ...
                  <form onSubmit={handleSubmit} className="space-y-8">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                      <div className="space-y-2">
                        <label className="text-[10px] tracking-widest text-brand-gray uppercase block font-bold">Name</label>
                        <input 
                          type="text" 
                          required
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px] tracking-widest text-brand-gray uppercase block font-bold">Email</label>
                        <input 
                          type="email" 
                          required
                          className="w-full bg-transparent border-b border-brand-black/20 py-3 focus:outline-none focus:border-brand-black transition-colors" 
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] tracking-widest text-brand-gray uppercase block font-bold">Subject</label>
                      <input 
                        type="text" 
                        required
                        className="w-full bg-transparent border-b border-brand-black/20 py-3 focus:outline-none focus:border-brand-black transition-colors" 
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] tracking-widest text-brand-gray uppercase block font-bold">Message</label>
                      <textarea 
                        required
                        rows={5}
              ...
              {/* Map Section */}
              <div className="aspect-video bg-brand-black/5 overflow-hidden rounded-2xl shadow-sm">
                <iframe 
                  src={`https://maps.google.com/maps?q=${encodeURIComponent(settings?.address || "〒150-0001 東京都渋谷区神宮前6-33-1")}&t=&z=15&ie=UTF8&iwloc=&output=embed`}
                  width="100%" 
                  height="100%" 
                  style={{ border: 0 }} 
